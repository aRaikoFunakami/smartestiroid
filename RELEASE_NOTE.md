...
前回リリース v0.1.0 (2025-12-07) からの変更点をまとめています。
...
#### 統合的判断による目標達成判定
- 目標達成の判定を「プラン進捗のみ」から「3つの観点を統合的に判断」する方式に変更
  - 観点1: プラン進捗（✅完了マーク）
  - 観点2: 実行済みステップの詳細情報（各ステップの実行結果）
  - 観点3: 現在の画面状態（目標が達成された状態になっているか）
- Executorが「先読み」して複数操作を一度に実行した場合も正しく判定

#### ダイアログモード状態管理の改善
- ブロッキングダイアログモード時の状態管理を修正
- ダイアログ処理中の目標ステップ進行の正確な制御
- 詳細設計ドキュメント（`docs/dialog_mode_design.md`, `docs/dialog_mode_flow.md`）を追加

#### Allure レポート表示の改善
- replan 時の計画ステップをわかりやすく表示
- executor.json に smartestiroid のバージョン情報を保存
- Executor 実行後のスクリーンショットを添付

---

### ✨ 機能追加

- **verify_screen_content**: `analyze_screen_content` を置き換え、オブジェクトの存在/非存在を確認可能に
- **ステップ履歴の活用**: `analyze_state` で実行済みステップの詳細情報を参照
- **最初のアプリ起動スキップ**: テストシート設定で初回アプリ起動をスキップ可能に
...
#### 要素特定の精度向上
- 同一 `resource-id` を持つ複数要素がある場合、必ず XPath で特定するよう修正
- クリックミスなどの誤操作を防止

#### プロンプトチューニング
- Executor の verify を助けるプロンプト改善
- 目標ステップを不用意に分割しないよう制御
- 条件不成立ステップの評価ロジックを修正

#### 実行プランの改善
- `_generate_new_plan` で作成する実行プランを明確化
- 実行プランのリプランが可能なよう修正
- ツールを使う計画を立てるよう修正

#### パフォーマンス改善
- スクリーンショットの保存形式を PNG から JPG へ変更（ファイルサイズ削減）
- リトライ回数を3回まで許容
...
- **先読み実行問題**: Executor が `send_keys` と `press_keycode(ENTER)` を1つの処理で実行した場合、残りステップを DISCARD するよう修正
- **find_element エラー**: LLM が類推したセレクターによるエラーを回避
- **ダイアログモード状態**: ブロッキングダイアログモード時の状態管理問題を修正

---

### 📝 ドキュメント

- **docs/dialog_mode_design.md**: ダイアログモード分離設計書を追加
- **docs/dialog_mode_flow.md**: ダイアログモードフロー説明書（mermaid図付き）を追加
- **AGENTS.md**: 設計ドキュメントへの参照を追加
