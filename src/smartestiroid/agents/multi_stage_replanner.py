...
【★超重要★ 目標ステップの達成判定ルール - 統合的判断】
目標達成の判定は、以下の3つの観点を**統合的に**判断すること：
...
【統合判断の基準】
- プラン進捗が低い（例: 1/10）のに画面がOKに見える
  → たまたま同じような画面が見えているだけの可能性。慎重に判断
  → 実行済みステップの詳細情報を確認し、必要な操作が行われたか検証

- プラン進捗が高い（例: 8/10, 9/10）かつ画面状態がOK
  → 実行済みステップの詳細情報を確認
  → 必要な処理が全て実施されていると判断できれば、残りステップは不要（DISCARD）
  → Executorが「先読み」して複数操作を一度に実行した可能性を考慮
...
- 「すべてのタブをタップする」「すべてのボタンを押す」等の目標の達成判断:
  ...
  2. 「実行済みステップの詳細情報」で各操作が実行されたか確認する
  3. プラン内の対象操作（各タブのタップ等）が全て ✅（完了済み）かを確認
  4. 全て完了済み → current_objective_achieved = True
  5. ▶️（現在位置）や ⏳（未実行）の対象操作がある → 画面状態と実行履歴も確認
  
- ★重要★ 「すべて」目標では、プランの進捗と実行履歴の両方を確認すること
  - プランで全タブのタップが✅なら、現在どのタブが選択されていても「達成」
  - プランで「お気に入りタブをタップ」が⏳でも、実行履歴で実行済みなら達成の可能性あり
...
8. 【★重要★ 残りステップの処理方針】（残りステップがある場合のみ）
   ...
   【手順A】3つの観点を確認する：
   1. プラン進捗（✅完了、▶️現在、⏳未実行）
   2. 実行済みステップの詳細情報（各ステップで何が行われたか）
   3. 現在の画面状態（目標達成後の期待される状態になっているか）
   
   【手順B】統合的に判断する：
   - 画面状態が目標達成を示しており、かつ実行済みステップで必要な操作が行われている
     → remaining_steps_action = "DISCARD"（残りステップは不要）
   
   - 例: 目標「URLを入力してエンターキーで確定」、プラン進捗 2/3
     → 実行済みステップで「send_keys」と「press_keycode(ENTER)」が一度に実行された
     → 画面にはYahooトップページが表示されている
     → 残りの「エンターキーを押す」は重複操作になる → DISCARD
   
   - 画面状態が目標未達成を示している
     → remaining_steps_action = "EXECUTE"（残りステップを実行）
   
   - 画面状態と残りステップが合わない
     → remaining_steps_action = "REPLAN"（修正が必要）
