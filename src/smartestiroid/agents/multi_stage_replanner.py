...
【★重要★ テスト合否判定基準に基づいた条件分岐ステップの生成】
- テスト合否判定基準に「〇〇の場合は△△」「初期起動以外は〜しない」などの条件が含まれる場合、条件分岐ステップとして生成すること
- 例1: 目標「ダイアログの同意ボタンを押す」、基準「初期起動以外はダイアログ非表示」
  → 生成すべきステップ: 「ダイアログが表示されていれば同意ボタンを押す（条件分岐）」
  → 誤った生成: 「ダイアログが表示されているか確認する」だけでは不十分
- 条件分岐ステップは必ず「〜であれば〜する（条件分岐）」の形式で記述すること
