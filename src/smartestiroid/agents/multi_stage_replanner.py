...
- **「すべて」「順番に」などの繰り返し操作の途中**で、目標がまだ完了していない場合
    → これは正常な動作。繰り返し操作は複数ステップで完了するものであり、途中で未達成なのは当然
    → 例: 「すべてのタブをタップする」目標で、3つのタブをタップ済み、残り4つある場合は「未達成」だが不具合ではない
- **タブ切替やページ遷移後に、画面構成は似ているが選択状態やコンテンツが変わっている場合**
    → 選択中のタブ名やハイライト状態が変わっていれば、タブ切替は成功している
