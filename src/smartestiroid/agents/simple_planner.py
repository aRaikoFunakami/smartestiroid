...
0. **★最重要★ 最初のステップの特殊ルール（アプリ起動の自動除外）**
   - 最初（1つ目）のステップが「アプリを起動する」「アプリを実行する」「アプリを開く」など、
     **アプリ名やアプリID、APK名を明示せずに**アプリ起動を指示している場合は、そのステップを**除外する**
   - 理由: Appiumのcapabilitiesで指定されたアプリは自動起動されるため
   - ✅除外すべき例:
     - 「1. アプリを起動する」→ 除外（アプリ名が不明）
     - 「1. アプリを起動 2. ホームタブをタップ」→ 「ホームタブをタップする」のみ（1つ目を除外）
     - 「1. テスト対象アプリを起動する」→ 除外（具体的なアプリ名がない）
   - ❌除外してはいけない例:
     - 「1. Chromeを起動する」→ 含める（アプリ名が明示されている）
     - 「1. com.example.app を起動する」→ 含める（アプリIDが明示されている）
     - 「1. sample.apk をインストールして起動する」→ 含める（APK名が明示されている）
     - 「1. YouTubeアプリを開く」→ 含める（アプリ名が明示されている）
   - ★重要: このルールは1つ目のステップにのみ適用。2つ目以降の「アプリを起動する」は除外しない
...
2. **★最重要★ ステップ数は元の数に厳密に合わせる**
   - 入力に6ステップあれば、出力も6ステップ
   - 「1. ○○ 2. ○○ 3. ○○」なら3ステップ
   - **番号付きステップの粒度を尊重する**
     - 「2. yahoo.co.jp をURLバーに入力してエンターキーで確定する」→ これは1ステップ（分割禁止）
     - 「2. 設定画面を開く 3. Wi-FiをONにする」→ これは2ステップ（番号が分かれている）
   - **複数のステップを統合してはいけない**（例: 「確認する」と「押す」を「押す」だけにまとめない）

3. **期待結果（確認項目）のみ除外**
   - 「〇〇が表示されること」「〇〇であること」等の**期待結果の記述**は除外（別途処理される）
   - ただし、手順の「〇〇を確認する」というステップは**残す**（操作として必要）
   - ❌NG除外: 「1. ダイアログを確認する 2. ボタンを押す」→「ボタンを押す」（統合は禁止）
   - ✅OK除外: 「1. ボタンを押す 確認項目: ダイアログが閉じること」→「ボタンを押す」（期待結果は除外）
...
【出力例】
入力: "1. アプリを起動する 2. 利用規約ダイアログを確認する 3. 同意ボタンを押す"
出力: ["利用規約ダイアログを確認する", "同意ボタンを押す"]
（※ 1つ目の「アプリを起動する」はアプリ名がないため除外。2ステップになる）

入力: "1. Chromeを起動 2. yahoo.co.jpをURLバーに入力してエンターキーで確定する"
出力: ["Chromeを起動する", "yahoo.co.jpをURLバーに入力してエンターキーで確定する"]
（※ 「Chrome」というアプリ名があるため除外しない。2ステップ）

入力: "1. アプリを起動 2. ホームタブをタップ 3. 設定を開く"
出力: ["ホームタブをタップする", "設定を開く"]
（※ 1つ目の「アプリを起動」はアプリ名がないため除外。2ステップになる）

入力: "1. Chromeを起動 2. yahoo.co.jpに移動 確認項目: ページが表示されること"
出力: ["Chromeを起動する", "yahoo.co.jpに移動する"]
（※ 期待結果「確認項目」は除外、動作指示形で書く）
"""
